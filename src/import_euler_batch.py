#!/usr/bin/env python3
"""Batch dense import of MANY Euler vehicles in ONE S3-Select pass (WHERE vin IN ...).

The per-VIN import (import_euler_dense.py) samples the SAME partition days for every vehicle, so
scanning each sampled day's files once and selecting `vin IN (...)` extracts all requested VINs at
once — ~Nx cheaper than running the per-VIN import N times. Same day sampling (3 representative
days/month, all files) and 2023+ electrical column set.

Usage: python src/import_euler_batch.py <vin_list_file> [days_per_month] [max_workers]
  vin_list_file = one full VIN per line
  -> data/euler/dense/<VIN>.parquet   (one per VIN that returned rows)
"""
...
def main():
    ...
    print(f"DONE: {len(df):,} rows -> {got}/{len(VINS)} VINs written to data/euler/dense/", flush=True)
